...
Template._loginButtonsLoggedOutPasswordService.helpers({
  fields: function () {
    var loginFields = [
      {fieldName: 'username-or-email', fieldLabel: 'FIELD_login_username_or_email',
       visible: function () {
         return _.contains(
           ["USERNAME_AND_EMAIL", "USERNAME_AND_OPTIONAL_EMAIL"],
           passwordSignupFields());
       }},
      {fieldName: 'username', fieldLabel: 'FIELD_login_username',
       visible: function () {
         return passwordSignupFields() === "USERNAME_ONLY";
       }},
      {fieldName: 'email', fieldLabel: 'FIELD_login_email', inputType: 'email',
       visible: function () {
         return passwordSignupFields() === "EMAIL_ONLY";
       }},
      {fieldName: 'password', fieldLabel: 'FIELD_login_password', inputType: 'password',
       visible: function () {
         return true;
       }}
    ];
    ...
    var signupFields = [
      {fieldName: 'username', fieldLabel: 'FIELD_signup_username',
       visible: function () {
         return _.contains(
           ["USERNAME_AND_EMAIL", "USERNAME_AND_OPTIONAL_EMAIL", "USERNAME_ONLY"],
           passwordSignupFields());
       }},
      {fieldName: 'email', fieldLabel: 'FIELD_signup_email', inputType: 'email',
       visible: function () {
         return _.contains(
           ["USERNAME_AND_EMAIL", "EMAIL_ONLY"],
           passwordSignupFields());
       }},
      {fieldName: 'email', fieldLabel: 'FIELD_signup_email_optional', inputType: 'email',
       visible: function () {
         return passwordSignupFields() === "USERNAME_AND_OPTIONAL_EMAIL";
       }},
      {fieldName: 'password', fieldLabel: 'FIELD_signup_password', inputType: 'password',
       visible: function () {
         return true;
       }},
      {fieldName: 'password-again', fieldLabel: 'FIELD_signup_password_again',
       inputType: 'password',
       visible: function () {
...
Template._loginButtonsChangePassword.helpers({
  fields: function () {
    return [
      {fieldName: 'old-password', fieldLabel: 'FIELD_changepass_current', inputType: 'password',
       visible: function () {
         return true;
       }},
      {fieldName: 'password', fieldLabel: 'FIELD_changepass_new', inputType: 'password',
       visible: function () {
         return true;
       }},
      {fieldName: 'password-again', fieldLabel: 'FIELD_changepass_new_again',
       inputType: 'password',
       visible: function () {
         // No need to make users double-enter their password if
         // they'll necessarily have an email set, since they can use
  ...
  Meteor.loginWithPassword(loginSelector, password, function (error, result) {
    if (error) {
      	if (error.reason == 'User not found')
				  loginButtonsSession.errorMessage("ERROR_usernotfound")
				else if (error.reason == 'Incorrect password')
					loginButtonsSession.errorMessage("ERROR_incorrect_password")
				else 
  ...
  Accounts.createUser(options, function (error) {
    if (error) {
        if (error.reason == 'Email already exists.')
				  loginButtonsSession.errorMessage("ERROR_email_exists")
				else 
				  loginButtonsSession.errorMessage(error.reason || "Unknown error");
    } else {
      loginButtonsSession.closeDropdown();
  ...
  if (email.indexOf('@') !== -1) {
    Accounts.forgotPassword({email: email}, function (error) {
      if (error)
      {
        ...
        loginButtonsSession.infoMessage("INFO_forgotpass_emailsent");
    });
  } else {
    loginButtonsSession.errorMessage("ERROR_forgotpass_invalid_email");
  }
};
